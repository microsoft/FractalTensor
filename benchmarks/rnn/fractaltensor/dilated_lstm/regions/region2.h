...
void Region2DepthFirstReuse(const GPUContext& context, Tensor& hsss,
  ...
  int64_t stride_i = hsss.dim_size(-3) * hsss.dim_size(-2) * hsss.dim_size(-1);
  ...
  for (int64_t i = lb0; i < ub0; ++i) {
    ...
    auto times =
        LstmCell(context, x, w, c0, h0, u, out1, out2, tmp1, tmp2, tmp3, tmp4);
    ...
    for (int64_t j = lb1 + link_len; j < ub1; j += link_len) {
      ...
      auto times =
          LstmCell(context, x, w, c, h, u, out1, out2, tmp1, tmp2, tmp3, tmp4);
