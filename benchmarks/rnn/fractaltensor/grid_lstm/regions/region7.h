...
void Region7DepthFirst(const GPUContext& context, Tensor& hsss,
  ...
  /* ============ Region 7 is controlled by s single control
   ================
   (>0, 0, >0, *)
   [[1, 0, 1, 0]
    [0, 1, 0, 0]
    ...
    p = d           <--> j = m - p      // j -> trg_length
    q = k           <--> k = q          // k -> batch

    Loop bounds:
    2 <= m < D + TL - 1
    max(1, m - TL + 1) <= n < min(m, D)
    0 <= q < N
 ...
 */
  ...
  // Loop bounds: 2 <= m < D + TL - 1
  int64_t lb0 = 2;
  int64_t ub0 = depth_size + trg_length - 1;
  ...
  for (int64_t m = lb0; m < ub0; ++m) {
    // Loop bounds: max(1, m - TL + 1) <= n < min(m, D)
    int64_t lb1 = max(1l, m - trg_length + 1l);
    ...
    for (int64_t p = lb1; p < ub1; ++p) {
      for (int64_t q = lb2; q < ub2; ++q) {
        // state_y
        gather_indices_h_cpu[0][count * end_size_h] = p;
        gather_indices_h_cpu[0][count * end_size_h + 1] = 0;
        gather_indices_h_cpu[0][count * end_size_h + 2] = m - p - 1;
        gather_indices_h_cpu[0][count * end_size_h + 3] = 1;
        gather_indices_h_cpu[0][count * end_size_h + 4] = q;

        // x_t,y_t
        gather_indices_h_cpu[1][count * end_size_h] = p - 1;
        gather_indices_h_cpu[1][count * end_size_h + 1] = 0;
        gather_indices_h_cpu[1][count * end_size_h + 2] = m - p;
        gather_indices_h_cpu[1][count * end_size_h + 3] = 0;
        gather_indices_h_cpu[1][count * end_size_h + 4] = q;

        gather_indices_h_cpu[2][count * end_size_h] = p - 1;
        gather_indices_h_cpu[2][count * end_size_h + 1] = 0;
        gather_indices_h_cpu[2][count * end_size_h + 2] = m - p;
        gather_indices_h_cpu[2][count * end_size_h + 3] = 1;
        gather_indices_h_cpu[2][count * end_size_h + 4] = q;

        // h_x,h_y
        gather_indices_h_cpu[3][count * end_size_h] = p;
        gather_indices_h_cpu[3][count * end_size_h + 1] = 0;
        gather_indices_h_cpu[3][count * end_size_h + 2] = m - p;
        gather_indices_h_cpu[3][count * end_size_h + 3] = 0;
        gather_indices_h_cpu[3][count * end_size_h + 4] = q;

        gather_indices_h_cpu[4][count * end_size_h] = p;
        gather_indices_h_cpu[4][count * end_size_h + 1] = 0;
        gather_indices_h_cpu[4][count * end_size_h + 2] = m - p;
    ...
    compute[1] += times[1];
#ifdef DEBUG_ON
    std::cout << "wave " << m << " numel: " << numel << std::endl;
    std::cout << "VanillaRNNCellBMM finish " << times[0] << " " << times[1]
              << std::endl;
#endif
    CHECK_ERROR(cudaGetLastError());
...
#ifdef DEBUG_ON
  std::cout << "region7: hsss:" << std::endl;
