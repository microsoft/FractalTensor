...
void Region8DepthFirst(const GPUContext& context, Tensor& hsss,
  ...
  for (int64_t m = lb0; m < ub0; ++m) {
    ...
    for (int64_t n = lb1; n < ub1; ++n) {
      ...
      for (int64_t p = lb2; p < ub2; ++p) {
        ...
        for (int64_t q = lb3; q < ub3; ++q) {
          ...
          gather_indices_h_cpu[3][count * end_size_h + 2] = m - n - p - 1;
    ...
    CudaCheck(
        cudaMalloc(&gather_indices_b, numel * end_size_w * sizeof(int64_t)));
    ...
    CudaCheck(cudaMemcpy(input_dims_ws, ws.dims().data(),
                         ws.ndim() * sizeof(int64_t), cudaMemcpyHostToDevice));
    ...
    CudaCheck(cudaMemcpy(input_dims_us, us.dims().data(),
                         us.ndim() * sizeof(int64_t), cudaMemcpyHostToDevice));
    ...
    CudaCheck(cudaMemcpy(input_dims_bs, bs.dims().data(),
                         bs.ndim() * sizeof(int64_t), cudaMemcpyHostToDevice));
    ...
    Tensor state_x({numel / batch_size, batch_size, hidden_size}, allocator);
    Tensor state_y({numel / batch_size, batch_size, hidden_size}, allocator);
    FillZeros(state_x);
    FillZeros(state_y);

    Tensor state({numel / batch_size, batch_size, 2 * hidden_size}, allocator);
    FillZeros(state);
