...
float run_dilated_region1(const int depth, const int seq_length,
  ...
  for (auto i = 0; i < repeat; i++) {
    elapsed_time += cuda_kernel::DilatedLstmRegion1<Element, InstructionShape,
                                                    ValueMnk, WarpArragement,
                                                    CtaTileShape, WholeShape>(
        dHsss, dCsss, dXs, dWs, dUs, init, seq_length);
  }
...
float run_dilated_lstm_cell(const int depth, const int seq_length,
  ...
  cuda_kernel::CuteLSTMCell<Element, InstructionShape, ValueMnk, WarpArragement,
                            CtaTileShape, WholeShape>
      lstm_cell;
  float elapsed_time = 0.0f;
  ...
  if (depth >= 2) {
    elapsed_time +=
        run_dilated_lstm_cell<cutlass::half_t, TileShape<2, 2, 1>,
                              TileShape<16 * 4, 32 * 2, 32 * 2>, CellShape2>(
            depth, seq, batch, hidden, i2, fout);
  }

  if (depth >= 3) {
    elapsed_time +=
        run_dilated_lstm_cell<cutlass::half_t, TileShape<2, 2, 1>,
                              TileShape<16 * 4, 32 * 2, 32 * 2>, CellShape3>(
            depth, seq, batch, hidden, i3, fout);
  }

  if (depth >= 4) {
    elapsed_time +=
        run_dilated_lstm_cell<cutlass::half_t, TileShape<2, 2, 1>,
                              TileShape<16 * 4, 32 * 2, 32 * 2>, CellShape4>(
            depth, seq, batch, hidden, i4, fout);
  }

  if (depth >= 5) {
    elapsed_time +=
        run_dilated_lstm_cell<cutlass::half_t, TileShape<2, 2, 1>,
                              TileShape<16 * 4, 32 * 2, 32 * 2>, CellShape5>(
            depth, seq, batch, hidden, i5, fout);
  }

  if (depth >= 6) {
    elapsed_time +=
        run_dilated_lstm_cell<cutlass::half_t, TileShape<2, 2, 1>,
                              TileShape<16 * 4, 32 * 2, 32 * 2>, CellShape6>(
            depth, seq, batch, hidden, i6, fout);
  }
