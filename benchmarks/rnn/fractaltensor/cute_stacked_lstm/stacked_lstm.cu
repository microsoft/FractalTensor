...
float bench_stacked_lstm(const int depth, const int seq_length,
  ...
  for (auto i = 0; i < repeat; ++i) {
    elapsed_time += cuda_kernel::StackedLstmRegion1<Element, InstructionShape,
                                                    ValueMnk, WarpArragement,
                                                    CtaTileShape, WholeShape>(
        dHsss, dCsss, dXs, dWs, dUs, depth, seq_length, batch_size,
        hidden_size);

    elapsed_time += cuda_kernel::StackedLstmRegion2<Element, InstructionShape,
                                                    ValueMnk, WarpArragement,
                                                    CtaTileShape, WholeShape>(
        dHsss, dCsss, dXs, dWs, dUs, depth, seq_length, batch_size,
        hidden_size);
  ...
  float cudnn_time = bench_cudnn_lstm_half(batch_size, hidden_size, seq_length,
                                           depth, input_size);
