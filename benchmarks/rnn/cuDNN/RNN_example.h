...
void RNNSample<T_ELEM>::setup(RNNSampleOptions& options) {
  ...
  switch (options.cellMode) {
    case 0:
      ...
      snprintf(cellModeEnumValue, sizeof(cellModeEnumValue), "CUDNN_RNN_RELU");
      break;
    case 1:
      cellMode = CUDNN_RNN_TANH;
      snprintf(cellModeEnumValue, sizeof(cellModeEnumValue), "CUDNN_RNN_TANH");
      break;
    case 2:
      cellMode = CUDNN_LSTM;
      snprintf(cellModeEnumValue, sizeof(cellModeEnumValue), "CUDNN_LSTM");
      break;
    case 3:
      cellMode = CUDNN_GRU;
  ...
  switch (options.dataType) {
    case 0:
      ...
      snprintf(dataTypeEnumValue, sizeof(dataTypeEnumValue), "CUDNN_DATA_HALF");
      break;
    case 1:
  ...
  for (int layer = 0; layer < numLayers * bidirectionalScale; layer++) {
    for (int linLayerID = 0; linLayerID < numLinearLayers; linLayerID++) {
      ...
      cudnnErrCheck(cudnnGetRNNWeightParams(
          cudnnHandle, rnnDesc, layer, weightSpaceSize, weightSpace, linLayerID,
          wDesc, (void**)&linLayerMat, bDesc, (void**)&linLayerBias));

      if (linLayerMat) {
        cudnnErrCheck(cudnnGetTensorNdDescriptor(wDesc, 3, &dataTypeTemp,
                                                 &nbDims, dim, stride));
      ...
      if (linLayerBias) {
        cudnnErrCheck(cudnnGetTensorNdDescriptor(bDesc, 3, &dataTypeTemp,
                                                 &nbDims, dim, stride));
...
void RNNSample<T_ELEM>::run() {
  ...
  for (int i = 0; i < 3; ++i) {
    cudnnErrCheck(cudnnRNNForward(
        ...
        weightSpace, workSpaceSize, workSpace, reserveSpaceSize, reserveSpace));
  }
  ...
  // *********************************************************************************************************
  {
    ...
    cudaErrCheck(cudaMemcpy(testOutputy, y, outputTensorSize * sizeof(T_ELEM),
                            cudaMemcpyDeviceToHost));
    if (hy != NULL) {
    ...
    for (int m = 0; m < miniBatch; m++) {
      ...
      for (int j = 0; j < seqLength; j++) {
        for (int i = 0; i < hiddenSize * bidirectionalScale; i++) {
          localSumi += (double)
              testOutputy[j * miniBatch * hiddenSize * bidirectionalScale +
      ...
      for (int j = 0; j < numLayers * bidirectionalScale; j++) {
        for (int i = 0; i < hiddenSize; i++) {
          if (hy != NULL) {
            localSumh += (double)
                testOutputhy[j * hiddenSize * miniBatch + m * hiddenSize + i];
          }
          if ((cy != NULL) && (cellMode == CUDNN_LSTM)) {
            localSumc += (double)
                testOutputcy[j * hiddenSize * miniBatch + m * hiddenSize + i];
          }
        }
      }
