...
float StackedLstmRegion3(Element* hsss, Element* csss, const Element* xss,
  ...
  for (int m = lb3; m < ub3; ++m) {
    ...
    // cpu
    int64_t* host_indices = (int64_t*)malloc(remain_size * k * sizeof(int64_t));
    ...
    cudaMemcpy(device_indices, host_indices, remain_size * k * sizeof(int64_t),
