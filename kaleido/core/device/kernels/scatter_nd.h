  ...
  for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x,
    ...
    if (tid < n) {
      ...
      for (size_t offset = 0; offset < slice_size; ++offset) {
        atomicAdd(out + out_index + offset, in[tid * slice_size + offset]);
      }
    }
  }
}
...
template <typename T>
struct ScatterNDOp {
  void operator()(Tensor& data, const Tensor& updates, const Tensor& indices) {
...
struct ScatterNDOpv2 {
  void operator()(T* data, const T* updates, const int64_t* indices,
    ...
    // `n` is the product of all dimensions excluding the innermost
    // dimension of `indices`.
    size_t n = std::accumulate(indices_dims.begin(), indices_dims.end() - 1, 1,
    ...
    size_t data_size = std::accumulate(data_dims.begin(), data_dims.end(), 1,
                                       std::multiplies<int64_t>());
