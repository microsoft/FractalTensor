...
#include "kaleido/core/device/kernels/online_softmax.h"
...
template <typename T>
class OnlineNormalizedSoftmaxOp<GPUContext, CUDAPlace, T> {
public:
  void operator()(const GPUContext& context, const Tensor& x, Tensor& y,
    ...
    cuda_kernel::KeOnlineNormalizedSoftMax<<<grid, block, 0>>>(
        x.data<T>(), y.mutable_data<T>(), width);
  }
};

template class OnlineNormalizedSoftmaxOp<GPUContext, CUDAPlace, float>;
