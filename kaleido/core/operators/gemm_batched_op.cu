...
public:
  void operator()(const GPUContext& context, const std::vector<Tensor>& A,
    ...
    for (int i = 0; i < A.size(); ++i) {
      CHECK(A[i].layout() == B[i].layout() && A[i].layout() == C[i].layout())
    ...
    switch (A[0].layout()) {
      case Layout::COL_MAJOR: {
        ...
        gemm(handle, transa, transb, m, n, k, alpha, (const T**)devAarray, lda,
             (const T**)devBarray, ldb, beta, (T**)devCarray, ldc, batch_count);
        break;
      }
      case Layout::ROW_MAJOR: {
        ...
        gemm(handle, transb, transa, n, m, k, alpha, (const T**)devBarray, ldb,
             (const T**)devAarray, lda, beta, (T**)devCarray, ldc, batch_count);
        break;
      }
      default:
