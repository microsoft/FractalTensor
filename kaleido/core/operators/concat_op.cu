...
public:
  void operator()(const GPUContext& context, const std::vector<Tensor>& inputs,
                  Tensor& output, size_t dim) {
    ...
    if (!has_same_shape || num_input < 2 || num_input > 4) {
      ...
      CudaCheck(cudaMemcpy(dev_inputs_data, inputs_data, num_input * sizeof(T*),
    ...
    if (has_same_shape) {
      if (num_input == 2) {
        ...
        ConcatKernel<<<grid_dims, block_dims, 0>>>(
            ...
            out_col, output.mutable_data<T>());
      } else if (num_input == 4) {
        ConcatKernel<<<grid_dims, block_dims, 0>>>(
            inputs_data[0], inputs_data[1], inputs_data[2], inputs_data[3],
            in_col, out_row, out_col, output.mutable_data<T>());
      } else {
        ConcatKernel<<<grid_dims, block_dims, 0>>>(dev_inputs_data, num_input,
                                                   in_col, out_row, out_col,
                                                   output.mutable_data<T>());
      ...
      CudaCheck(
          cudaMalloc(&dev_inputs_col_data, inputs_col_num * sizeof(int64_t)));
