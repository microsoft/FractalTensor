...
void CheckInput(const Tensor& input, const Tensor& output,
  ...
  if (input.ndim() != output.ndim() || input.ndim() != dims.size() ||
    ...
    LOG(FATAL) << "Expect the dimensions of input tensor, output tensor and "
  ...
  for (int i = 0; i < input.ndim(); ++i) {
    ...
    if (d == 0) LOG(FATAL) << "Do not support tensor dimensions with size 0.";
    if (input.dim_size(dims[i]) != d)
      LOG(FATAL) << "The output shape should be the same as the tansposed "
                    "input shape.";
  }
}
...
public:
  void operator()(const Tensor& input, Tensor& output,
    ...
    cudaMemcpy(stride, stride_host.data(), dims.size() * sizeof(int64_t) * 2,
    ...
    if (dims.size() == 2) {
      dim3 dimGrid(input.dim_size(0) / TILE_DIM, input.dim_size(1) / TILE_DIM,
                   1);
      dim3 dimBlock(TILE_DIM, BLOCK_ROWS, 1);
      TransposeKernel2DNoConflict<T><<<dimGrid, dimBlock>>>(in_data, out_data);
    } else {
