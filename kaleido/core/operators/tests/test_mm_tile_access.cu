...
int TestGemmAccess(const Element* dA, Element* dA_copy, const Element* dB,
  ...
  using LoadC = GmemTileTransmitter<kTM, kTN, Element, kThreads,
  ...
  TestCopy<kThreads, Element, kM, kN, kK, kTM, kTN, kTK, decltype(loader_a),
      ...
      const_cast<Element*>(dC), dC_copy, loader_a, storer_a, loader_b, storer_b,
      loader_c, storer_c);
  return 0;
}
...
void run_test() {
  ...
  std::shared_ptr<Tensor> dA =
      SequentialGpuTensor<Element>({WholeShape::kM, WholeShape::kK}, allocator);
  ...
  std::shared_ptr<Tensor> dB =
      SequentialGpuTensor<Element>({WholeShape::kK, WholeShape::kN}, allocator);
  std::shared_ptr<Tensor> dB_copy = ConstantGpuTensor<Element>(
      {WholeShape::kK, WholeShape::kN}, allocator, 0.);

  std::shared_ptr<Tensor> dC =
      SequentialGpuTensor<Element>({WholeShape::kM, WholeShape::kN}, allocator);
