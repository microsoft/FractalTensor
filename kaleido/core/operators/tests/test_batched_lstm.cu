...
void run_dynamic_fused_batched_lstm_cell(const int m, const int n, const int k,
  ...
  cute_fused_bmm_lstm_cell(
      ...
      csss.data<Element>(), hsss.data<Element>(), c_out.mutable_data<Element>(),
      h_out.mutable_data<Element>(), depth, seq_length, hidden_size, b,
      stride_a, stride_b, stride_c, m, n, k, b);
