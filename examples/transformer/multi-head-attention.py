...
def f0(
    x: Tensor['1, 256', float, 'cpu'], wxs: FractalTensor[Tensor['1, 256',
                                                                 float, 'cpu']]
) -> Tensor['1, 32', float, 'cpu']:
...
def f2(
    states: FractalTensor[Tensor['1, 256', float, 'cpu']],
    os: FractalTensor[Tensor['1, 256', float, 'cpu']], wo: Tensor['32, 256',
                                                                  float, 'cpu']
...
def f1(
    oss: FractalTensor[FractalTensor[Tensor['1, 256', float, 'cpu']]],
    wos: FractalTensor[Tensor['1, 256', float, 'cpu']]
) -> FractalTensor[Tensor['1, 256', float, 'cpu']]:
    # reduction over the num_head dimension
    vs = ops.reduce(lambda s, xs: f2(s, *xs),
                    ops.zip(oss, wos),
...
def mha(
    qss: FractalTensor[FractalTensor[Tensor['1, 256', float, 'cpu']]],
    ...
    wos: FractalTensor[Tensor['32, 256', float, 'cpu']]
) -> FractalTensor[FractalTensor[FractalTensor[Tensor['1, 256', float,
                                                      'cpu']]]]:
...
if __name__ == '__main__':
    ...
    # depth-1: batch_size
    # depth-2: seq_len
    qss = create_input(hidden_dim=hidden_dim,
                       seq_len=seq_len,
                       batch_size=batch_size)
    kss = create_input(hidden_dim=hidden_dim,
                       seq_len=seq_len,
                       batch_size=batch_size)
    vss = create_input(hidden_dim=hidden_dim,
                       seq_len=seq_len,
                       batch_size=batch_size)
