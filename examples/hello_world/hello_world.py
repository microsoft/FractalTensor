...
def f2(
    ...
    ys = ops.scan(lambda s, x: f3(x, s, w, u),
                  xs,
                  initializer=ops.zeros(shape=(1, 512),
                                        device='cpu',
                                        dtype='float'))
    return ys
...
def f1(
    xs: FractalTensor[Tensor['1, 512', float, 'cpu']],
    Ws: FractalTensor[Tensor['512, 512', float,
                             'cpu']], Us: FractalTensor[Tensor['512, 512',
                                                               float, 'cpu']]
) -> FractalTensor[FractalTensor[Tensor['1, 512', float, 'cpu']]]:
    yss = ops.scan(lambda state, x: f2(state, *x),
                   ops.zip(Ws, Us),
                   initializer=xs)
    return yss
...
@kaleido.function(ctx)
def f(
    xss: FractalTensor[FractalTensor[Tensor['1, 512', float,
                                            'cpu']]], params: Params
) -> FractalTensor[FractalTensor[FractalTensor[Tensor['1, 512', float,
                                                      'cpu']]]]:
